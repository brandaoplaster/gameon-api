...
RSpec.describe "Admin V1 Products as :admin", type: :request do
    ...
    context "with valid params" do
      ...
      let(:product_params) do 
        { 
          ...
          .merge(category_ids: categories.map(&:id))
  ...
  context "PATCH /products/:id" do
    ...
    let(:system_requirement) { create(:system_requirement) }
    let(:url) { "/admin/v1/products/#{product.id}" }
    let(:patch_header) { auth_header(logged_in_user, merge_with: { 'Content-Type' => 'multipart/form-data' }) }
      ...
      let(:product_params) do 
        {
          ...
          .merge(category_ids: new_categories.map(&:id)) 
        }
      end
